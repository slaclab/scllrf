...
sed 's|record(.*"$(P)\(.*\)_W"|alias( "$(P)\1_W", "$(P)\L\1_ao"|' GUNBRegisters.template | grep alias > GUNBRegisterAlias.template
...
sed 's|record(.*"$(P)\(.*\)_W"|alias( "$(P)\1_W", "$(P)\L\1_bo"|' GUNBRegisters.template | grep alias >> GUNBRegisterAlias.template
# For all readable records, make an alias in lower case, and drop the _R
sed 's|record(.*"$(P)\(.*\)_R"|alias( "$(P)\1_R", "$(P)\L\1"|' GUNBRegisters.template | grep alias >> GUNBRegisterAlias.template
